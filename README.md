...
## グランビルの法則による「買い時/売り時だった日」の可視化

`analysis.py` の `compute_granville_signals()` で、90日移動平均線（長期）・
30日移動平均線（中期）・日々の株価（短期/日次）それぞれの向き
（上昇/横ばい/下降）の組み合わせを**日ごとに**計算し、あらかじめ指定した
判定表（`GRANVILLE_TABLE`）に明確に一致する日だけを「買い」「売り」として
抽出しています。

- 判定テーブルは、3方向×3方向×3方向＝27通りの組み合わせのうち、
  あらかじめ指定された15通りだけを定義したものです。表にない残り12通りの
  組み合わせの日は、無理に多数決などで判定せず「マークしない」扱いに
  しています（あいまいな日を強引に買い/売りに分類しないという方針のため）。
  挙動を変えたい場合は `GRANVILLE_TABLE` に該当の組み合わせを追記して
  ください。
- 各トレンドの「上昇/横ばい/下降」は、`trend_direction_series()` が
  「その時点の値」と「N営業日前(有効値ベース)の値」を比較して判定します
  （長期=10営業日前、中期=5営業日前、短期/日次=2営業日前と比較）。
  この比較日数や、横ばいとみなす変化率のしきい値（デフォルト0.5%、
  日次のみ0.3%）は `compute_signal()` 内で調整できます。
- フロントエンドでは、グラフの表示期間（最大180日）内で判定基準に
  明確に一致した日を、緑（買い）・赤（売り）の点としてグラフ上に
  マークします。特定の1日（最新日など）だけを強制的に判定するのではなく、
  期間内のすべての該当日を表示する方式です。

## もしシグナル通りに売買していたら（バックテスト）

`analysis.py` の `simulate_granville_backtest()` で、「買いシグナルの日に1株買い、
売りシグナルの日に1株売っていたら、最新日時点でどうなっているか」を
シミュレーションしています。

- 買い/売りシグナルを、出現した順にFIFO（先入れ先出し）方式でペアリングします。
  例えば「買い→売り」の順で1組決済されると、その差額が確定損益になります。
- 買いシグナルの後に対応する売りシグナルが出現していない場合は、
  その株を「保有中」として、最新株価との差額を含み損益として計算します。
- 逆に、売りシグナルの後に対応する買いシグナルが出現していない場合は、
  株を借りて売った状態（空売り）が続いているとみなし、同様に含み損益を
  計算します（実際の信用取引の手数料・金利等は考慮していません）。
- 詳細画面（各銘柄カードをタップ）の「もしシグナル通りに売買していたら」
  セクションで、確定損益・含み損益・保有状況を確認できます。
