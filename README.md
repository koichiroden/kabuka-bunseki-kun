...
## グランビルの法則（8パターン）による買い時/売り時シグナル

`analysis.py` の `find_granville_candidates()` で、**30日移動平均線**と
**現在値（株価）** の位置関係・移動平均線自体の向きから、伝統的な
「グランビルの法則」に基づく買い4パターン・売り4パターン、合計8パターンを
日ごとに判定します。

- **買いシグナル**
  1. 横ばい/上向きの移動平均線を、現在値が下から上に抜けたとき
  2. 上向きの移動平均線を、現在値が一時的に下抜けたとき（押し目買い）
  3. 上向きの移動平均線に現在値が接近し、抜けずに反発したとき
  4. 下向きの移動平均線から現在値が大きく下に離れたとき（自律反発狙い）
- **売りシグナル**
  5. 横ばい/下向きの移動平均線を、現在値が一時的に上抜けたとき
  6. 下向きの移動平均線を、現在値が上から下に抜けたとき
  7. 下向きの移動平均線に現在値が接近し、抜けずに反落したとき
  8. 上向きの移動平均線から現在値が大きく上に離れたとき（自律反落狙い）

実装上のポイント:

- パターン①⑤の「横ばい」は、移動平均線が「下降から横ばいへ転じた（＝底の
  可能性、①）」のか「上昇から横ばいへ転じた（＝天井の可能性、⑤）」のかで
  意味が正反対になります。これを見分けるため、短期（5営業日）の向きが
  「横ばい」のときだけ、より長い期間（`GRANVILLE_LONG_LOOKBACK`、
  デフォルト15営業日）で見た大局的な向きを使って①か⑤かを判定しています
  （同じ日に①と⑤が両方出てしまうのを防ぐため）。
- パターン③④⑦⑧の「接近」「大きく離れた」は、移動平均線からの乖離率(%)を
  しきい値と比較して判定します。「接近」は `GRANVILLE_NEAR_PCT`
  （デフォルト±2%）、「大きく離れた」は `GRANVILLE_FAR_PCT`
  （デフォルト±8%）で調整できます。
- 「シグナルの強さスコア」は、移動平均線からの乖離率(%)の絶対値と、
  直近5営業日（`GRANVILLE_MOMENTUM_LOOKBACK`）の値動きの強さ(%)を
  合計したものです。銘柄ごとに、買い候補・売り候補それぞれをこのスコアが
  高い順に並べ、最大 `TOP_SIGNAL_COUNT`（デフォルト5）件までをシグナルとして
  残します。候補が5件に満たない場合は、無理に5件へ水増しせず、実際に存在する
  件数（0〜4件）だけをそのままシグナルとします。
- 同じ日に複数のパターンが同時に該当した場合は、その日・その方向（買い/売り）
  についてスコアが最も高いパターン1つだけを採用します。
- フロントエンドでは、上位シグナルとして残った日のうち、グラフの表示期間
  （最大180日）内に含まれるものだけを、緑（買い）・赤（売り）の点として
  グラフ上にマークします。各銘柄の詳細画面では、実際にシグナリングされた
  日付・株価・該当パターン（①〜⑧）の一覧（`granvilleSignalDetails`）を
  確認できます。
