...
## グランビルの法則による「買い時/売り時スコア」と上位シグナル抽出

`analysis.py` の `compute_granville_signals()` で、90日移動平均線（長期）・
30日移動平均線（中期）・日々の株価（短期/日次）それぞれの向き
（上昇/横ばい/下降）の組み合わせを**日ごとに**計算し、あらかじめ指定した
判定表（`GRANVILLE_TABLE`）に一致する日を候補として洗い出します。
そのうえで、候補日の「シグナルの強さスコア」が高い順に、**銘柄ごとに
買い・売りそれぞれ上位5日だけ**を実際のシグナルとして残します。

- 判定テーブルは、3方向×3方向×3方向＝27通りの組み合わせのうち、
  あらかじめ指定された15通り（買い7通り・売り8通り）だけを定義した
  ものです。表にない残り12通りの組み合わせの日は、候補にすら含めません
  （あいまいな日を強引に買い/売りに分類しないという方針のため）。
  挙動を変えたい場合は `GRANVILLE_TABLE` に該当の組み合わせを追記して
  ください。
- 各トレンドの「上昇/横ばい/下降」は、`trend_info_series()` が
...
- 「シグナルの強さスコア」は、長期・中期・短期それぞれの変化率(%)の
  絶対値を合計したものです。3つの時間軸すべてで値動きがはっきりしている
  日ほど、スコアが高くなります。
- 銘柄ごとに、買い候補・売り候補それぞれをこのスコアが高い順に並べ、
  最大 `TOP_SIGNAL_COUNT`（デフォルト5）件までをシグナルとして残します。
  候補が5件に満たない場合は、無理に5件へ水増しせず、実際に存在する件数
  （0〜4件）だけをそのままシグナルとします。この定数は `analysis.py`
  冒頭で調整できます。
