...
フロントエンド（`StockAnalyzerApp.jsx`）に以下の機能を追加しています。

- **お気に入り登録**: 各銘柄カード右上、および詳細画面ヘッダーの☆ボタンで
  お気に入りに登録できます。登録状態はブラウザの `localStorage`
  （キー: `kabuka-bunseki-kun:favorites`）に保存され、次回訪問時にも
  復元されます。サーバー側には保存されないため、別の端末やブラウザからは
  見えません。
- **並び替えオプション**:
  - 「シグナル日が近い順」: 各銘柄の買い/売りシグナルのうち、一番新しい
    （今日に近い）日付を基準に並べ替えます。シグナルが1件もない銘柄は
    末尾に表示されます。
  - 「お気に入り順」: お気に入り登録した銘柄を先頭に表示し、
    お気に入り同士は「おすすめ度」スコアで並べます。
